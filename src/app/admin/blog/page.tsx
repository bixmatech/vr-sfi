...
export default function AdminBlog() {
  const { data, mutate } = useSWR('/api/admin/blog', fetcher);
  const [title, setTitle] = useState('');
  const [slug, setSlug] = useState('');
  const [content, setContent] = useState('');

  const create = async () => {
    await fetch('/api/admin/blog', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title, slug, content, published: true }) });
  ...
  return (
    <main className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="border rounded p-4 space-y-2">
        ...
        <button className="px-4 py-2 bg-black text-white rounded" onClick={create}>Publish</button>
      </div>
